...
class MismatchIdentifier:
    ...
    def _detect_colors(self, image):
        ...
        detected_colors["red"] = detected_colors.pop("red1") + detected_colors.pop(
            "red2"
        )
    ...
    def classify_image(self, image_path):
        ...
        red_mask1 = cv2.inRange(
            hsv_image,
            np.array(self.color_ranges["red1"][0]),
            np.array(self.color_ranges["red1"][1]),
        )
        ...
        red_contours, _ = cv2.findContours(
            red_edges, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
        )
        ...
        for red_contour in red_contours:
            for green_contour in green_contours:
                ...
                distance_pixels = cv2.pointPolygonTest(
                    green_contour,
                    (int(red_contour[0][0][0]), int(red_contour[0][0][1])),
                    True,
                )
